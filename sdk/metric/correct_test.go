...
func TestViewCustomAggregator(t *testing.T) {
	...
	valuerecorder := Must(meter).NewFloat64ValueRecorder("name.exact")
	Must(meter).RegisterView(metric.NewView(valuerecorder.SyncImpl(), metric.Ungroup, nil, &testAggregatorFactory{}))
...
func TestViewBind(t *testing.T) {
	...
	Must(meter).RegisterView(metric.NewView(valuerecorder.SyncImpl(), metric.DropAll, nil, &testAggregatorFactory{}))
